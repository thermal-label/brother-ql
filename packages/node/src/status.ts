import { type PrinterStatus, type MediaType } from '@thermal-label/brother-ql-core';
...
export function parseStatus(bytes: Uint8Array): PrinterStatus {
  ...
  // Status type byte 14: 0x02 = error
  const statusType = bytes[14] ?? 0;
  const ready = errors.length === 0 && statusType !== 0x02;

  // Editor Lite mode: detected by mass-storage PID at discovery time;
  // not detectable from status bytes alone — passed in separately if needed
